...
export default function Settings({ onNavigate }: { onNavigate?: (page: Page) => void }) {
  ...
  const startGateway = useMutation({
    mutationFn: async () => {
      const result = await window.api.gateway.startShell()
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['gateway', 'health'] })
      queryClient.invalidateQueries({ queryKey: ['session-status'] })
      setRestartFeedback({ type: 'success', message: 'Gateway start command completed.' })
      restartFeedbackTimerRef.current = window.setTimeout(() => {
        setRestartFeedback(null)
      }, 7000)
    },
    onError: (error) => {
      ...
      setRestartFeedback({ type: 'error', message: `Start failed: ${message}` })
    }
  })
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-text-muted mt-1">OpenClaw is installed and updated independently of Pinchr.</p>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="secondary"
                      ...
                      disabled={restartGateway.isPending || startGateway.isPending}
                      className="gap-2"
                    >
                      <RotateCw
                        className={cn(
